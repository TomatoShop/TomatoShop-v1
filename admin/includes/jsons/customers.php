  ...
  class toC_Json_Customers {
    ...
    function listWishlists() {
      ...
      if (is_numeric($customers_id)) {
        $Qwishlists = $osC_Database->query('select wp.wishlists_products_id, wp.products_id_string, wp.date_added, wp.comments from :table_wishlists w, :table_wishlists_products wp where w.wishlists_id = wp.wishlists_id and w.customers_id = :customers_id');
        ...
        while($Qwishlists->next()) {
          $products_id = osc_get_product_id($Qwishlists->value('products_id_string'));
          $variants = osc_parse_variants_from_id_string($Qwishlists->value('products_id_string'));
