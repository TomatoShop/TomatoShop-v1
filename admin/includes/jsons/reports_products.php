...
require('includes/classes/category_tree.php');
  ...
  class toC_Json_Reports_products {
    ...
    function listLowStock() {
      ...
      $QlowStock = $osC_Database->query('select SQL_CALC_FOUND_ROWS p.products_id, pd.products_name, p.products_sku, p.products_status, p.products_quantity from :table_products p, :table_products_description pd where p.products_id = pd.products_id and pd.language_id = :language_id and p.products_quantity <= :stock_reorder_level');
      ...
      while ($QlowStock->next()) {
      	$records[] = array('products_id' => $QlowStock->ValueInt('products_id'),
                        	 'products_name' => $QlowStock->Value('products_name'),
      										 'products_sku' => $QlowStock->Value('products_sku'),
      					           'products_status' => $QlowStock->Value('products_status'),
