...
<style type="text/css">
/* page */
body { background-color: #ffffff; color: #000000; margin: 0px; font-family: Tahoma, Verdana, Arial, sans-serif; font-size: 10px; }
td, p { font-family: Tahoma, Verdana, Arial, sans-serif; font-size: 10px; }
H1, H1 a:link, H1 a:visited, H1 a:active, .pageHeading { font-family: Tahoma, Verdana, Arial, sans-serif; font-size: 18px; color: #727272; font-weight: bold; margin: 10px 0px; }
  ...
  <tr>
    <td><table border="0" width="100%" cellspacing="0" cellpadding="0">
      <tr>
        ...
        <td class="pageHeading" align="left"><?php echo osc_image('../images/store_logo.jpg', STORE_NAME); ?></td>
      </tr>
    </table></td>
  </tr>
  <tr>
...
<?php
    foreach ($osC_Order->getProducts() as $product) {
      ...
      echo '          <td align="left" valign="top">' . $osC_Tax->displayTaxRateValue($product['tax']) . '</td>' . "\n" .
           '          <td align="left" valign="top"><b>' . $osC_Currencies->format($product['final_price'], true, $osC_Order->getCurrency(), $osC_Order->getCurrencyValue()) . '</b></td>' . "\n" .
           '          <td align="left" valign="top"><b>' . $osC_Currencies->displayPriceWithTaxRate($product['final_price'], $product['tax'], 1, $osC_Order->getCurrency(), $osC_Order->getCurrencyValue()) . '</b></td>' . "\n" .
           '          <td align="left" valign="top"><b>' . $osC_Currencies->format($product['final_price'] * $product['quantity'], true, $osC_Order->getCurrency(), $osC_Order->getCurrencyValue()) . '</b></td>' . "\n" .
           '          <td align="left" valign="top"><b>' . $osC_Currencies->displayPriceWithTaxRate($product['final_price'], $product['tax'], $product['quantity'], $osC_Order->getCurrency(), $osC_Order->getCurrencyValue()) . '</b></td>' . "\n";
      echo '        </tr>' . "\n";
    }
?>
      </tbody>
...
<?php
  foreach ($osC_Order->getTotals() as $total) {
    echo '      <tr>' . "\n" .
         '        <td align="left">' . $total['title'] . '</td>' . "\n" .
         '        <td align="left">' . $total['text'] . '</td>' . "\n" .
         '      </tr>' . "\n";
  }
?>
    </table></td>
