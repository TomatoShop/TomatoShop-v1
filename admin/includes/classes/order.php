...
//start add by tomatoshop (bug : do not delete coupons after delete order)
      if ($error === false) {
        $Qgc = $osC_Database->query('delete from :table_coupons_redeem_history where orders_id = :orders_id');
        $Qgc->bindTable(':table_coupons_redeem_history', TABLE_COUPONS_REDEEM_HISTORY);
        $Qgc->bindInt(':orders_id', $id);
        $Qgc->setLogging($_SESSION['module'], $id);
        $Qgc->execute();
...
//end add by tomatoshop      
  ...
  function createOrder($data){
      ...
      $Qcustomer = $osC_Database->query('select c.customers_default_address_id, a.entry_company as company, a.entry_firstname as firstname, a.entry_lastname as lastname, a.entry_street_address as street_address, a.entry_suburb as suburb, a.entry_postcode as postcode, a.entry_city as city, a.entry_state as state, a.entry_country_id as country_id, a.entry_zone_id as zone_id, a.entry_telephone as telephone, co.countries_name as country, co.countries_iso_code_2 as country_iso2, co.countries_iso_code_3 as country_iso3, co.address_format as address_format, zo.zone_code as zone_code, zo.zone_name as zone_name from :table_customers c left join :table_address_book a on c.customers_default_address_id = a.address_book_id left join :table_countries co on a.entry_country_id = co.countries_id left join :table_zones zo on a.entry_zone_id = zo.zone_id where c.customers_id = :customers_id');
      ...
      $Qorder->bindValue(':delivery_state', $data['zone_name']);
      $Qorder->bindValue(':delivery_zone_id', $data['zone_id']);
      $Qorder->bindValue(':delivery_state_code', $data['zone_code']);
      ...
      $Qorder->bindValue(':delivery_telephone', $data['telephone']);
      ...
      $Qorder->bindValue(':billing_state', $data['zone_name']);
      $Qorder->bindValue(':billing_zone_id', $data['zone_id']);
      $Qorder->bindValue(':billing_state_code', $data['zone_code']);
      ...
      $Qorder->bindValue(':billing_telephone', $data['telephone']);
