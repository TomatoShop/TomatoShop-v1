...
*/
  ...
  if ($Qglobal->valueInt('global_product_notifications') !== 1) {
	...
	// start code for online transactions (persian Tomatocart)
	    $transactionid = '';
	    $Qtransaction = $osC_Database->query('select transaction_id from :table_orders_transactions_history where orders_id = :orders_id');
        $Qtransaction->bindTable(':table_orders_transactions_history', TABLE_ONLINE_TRANSACTIONS);
        $Qtransaction->bindInt(':orders_id', $Qorder->valueInt('orders_id'));
        $Qtransaction->execute();
		$transactionid = $Qtransaction->value('transaction_id');						
	// end code for online transactions (persian Tomatocart)   
?>
...
<div>
  <div style="float: right;"><?php echo osc_image('templates/' . $osC_Template->getCode() . '/images/order_process_success.png', $osC_Template->getPageTitle()); ?></div>
  ...
  <div style="padding-top: 30px;">
	...
	echo 
	"<span style='color:#cc0000; font-size:13px;' > " . $osC_Language->get('online_transaction_id') ." " .$transactionid. " </span>"
	  ;}
	?>
    <p>
