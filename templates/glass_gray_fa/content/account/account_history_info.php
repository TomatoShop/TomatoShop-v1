...
<div class="moduleBox">
  ...
  <h6><span style="float: left;"><?php echo $osC_Language->get('order_total_heading') . ' ' . $order->info['total']; ?></span><?php echo  $osC_Language->get('order_date_heading') . ' ' . osC_DateTime::getShort($order->info['date_purchased']) . ' <small>(' . $order->info['orders_status'] . ')</small>'; ?></h6>
...
?>
      ...
      <tr>
        ...
        <td align="left"><h6><?php echo $osC_Language->get('order_tax_title'); ?></h6></td>
        <td align="left"><h6><?php echo $osC_Language->get('order_total_title'); ?></h6></td>
      </tr>
...
<?php
  ...
  foreach ($order->products as $product) {
    echo '      <tr>' . "\n" .
         '        <td align="left" valign="top" width="30">' . $product['qty'] . '&nbsp;x&nbsp;</td>' . "\n" .
    ...
    if ( isset($product['customizations']) && !empty($product['customizations']) ) {
      echo '<p>';
        foreach ($product['customizations'] as $key => $customization) {
          echo '<div style="float: right">' . $customization['qty'] . ' x ' . '</div>';
    ...
    if (sizeof($order->info['tax_groups']) > 1) {
      echo '      <td valign="top" align="left">' . osC_Tax::displayTaxRateValue($product['tax']) . '</td>' . "\n";
    }

    echo '        <td align="left" valign="top">' . $osC_Currencies->displayPriceWithTaxRate($product['final_price'], $product['tax'], $product['qty'], $order->info['currency'], $order->info['currency_value']) . '</td>' . "\n" .
         '       </tr>' . "\n";
  }
?>
...
<?php
  foreach ($order->totals as $total) {
    echo '        <tr>' . "\n" .
         ...
         '         <td align="left" width="100">' . $total['text'] . '</td>' . "\n" .
         '       </tr>' . "\n";
  }
?>
