...
##                                                                           ##
##  $Id: enbank.php $                                                        ##
##  EGHTESAD NOVIN PAYMENT FOR PERSIAN TOMATOCART                            ##
##                                                                           ##
...
###############################################################################
  ...
  class osC_Payment_enbank extends osC_Payment {
    var $_title,
        $_code = 'enbank',
        $_status = false,
        $_sort_order,
    ...
    function osC_Payment_enbank() {
      ...
      $this->_status = (MODULE_PAYMENT_ENBANK_STATUS == '1') ? true : false;
      $this->_sort_order = MODULE_PAYMENT_ENBANK_SORT_ORDER;

      $this->form_action_url = 'https://pna.shaparak.ir/CardServices/controller';

      if ($this->_status === true) {
        if ((int)MODULE_PAYMENT_ENBANK_ORDER_STATUS_ID > 0) {
          $this->order_status = MODULE_PAYMENT_ENBANK_ORDER_STATUS_ID;
        }

        if ((int)MODULE_PAYMENT_ENBANK_ZONE > 0) {
          $check_flag = false;

          $Qcheck = $osC_Database->query('select zone_id from :table_zones_to_geo_zones where geo_zone_id = :geo_zone_id and zone_country_id = :zone_country_id order by zone_id');
          $Qcheck->bindTable(':table_zones_to_geo_zones', TABLE_ZONES_TO_GEO_ZONES);
          $Qcheck->bindInt(':geo_zone_id', MODULE_PAYMENT_ENBANK_ZONE);
    ...
    function confirmation() {
      ...
      $confirmation = array('title' => $this->_method_title,
                           'fields' => array(array('title' => $osC_Language->get('payment_enbank_description'))));
    ...
    function process_button() {
      ...
      $process_button_string = osc_draw_hidden_field('MID', MODULE_PAYMENT_ENBANK_MERCHANT_ID).
...
//	echo $soapclient->debug_str;
		...
		//	do{
	$res=  $soapProxy->verifyTransaction($RefNum,MODULE_PAYMENT_ENBANK_MERCHANT_ID);//reference number and sellerid	
			//		$i -= 1;
            //  } while((!$res) and ($i>0));
...
/********************************************** s t a t e   e r r o r ***********************************************/
function check_enbank_state_error($State){
	...
	switch($State){
		...
		case 'No Sufficient Funds' :
			$pay_error= "موجودي به اندازي کافي در حساب وجود ندارد.";
			break;
		case 'Issuer Down Slm' :
			$pay_error= "سيستم کارت بانک صادر کننده در وضعيت عملياتي نيست.";
			break;
		case 'TME Error' :
			$pay_error= "خطا ايجاد شده قابل شناسايى نيست. لطفا با مديريت سايت تماس بگيريد";
			break;
	}	
...
/************************************************* r e s   e r r o r ***********************************************/
function check_enbank_res_error($res){
	switch($res){
		case '1' :
