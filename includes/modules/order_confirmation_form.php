...
?>
              ...
              <tr>
                ...
                <td align="left"><b><?php echo $osC_Language->get('order_tax_title'); ?></b></td>
                <td align="left"><b><?php echo $osC_Language->get('order_total_title'); ?></b></td>
              </tr>
...
<?php
  ...
  foreach ($osC_ShoppingCart->getProducts() as $products) {
    echo '              <tr>' . "\n" .
         '                <td align="left" valign="top" width="30">' . $products['quantity'] . '&nbsp;x&nbsp;</td>' . "\n" .
...
<?php 
    ...
    if ($osC_ShoppingCart->numberOfTaxGroups() > 1) {
      echo '                <td valign="top" align="left">' . osC_Tax::displayTaxRateValue($osC_Tax->getTaxRate($products['tax_class_id'], $osC_ShoppingCart->getTaxingAddress('country_id'), $osC_ShoppingCart->getTaxingAddress('zone_id'))) . '</td>' . "\n";
    }

    echo '                <td align="left" valign="top">' . $osC_Currencies->displayPrice($products['final_price'], $products['tax_class_id'], $products['quantity']) . '</td>' . "\n" .
         '              </tr>' . "\n";
  }
?>
...
//    foreach ($osC_OrderTotal->getResult() as $module) {
    foreach ($osC_ShoppingCart->getOrderTotals() as $module) {
      echo '              <tr>' . "\n" .
           ...
           '                <td align="left" width="30%">' . $module['text'] . '</td>' . "\n" .
           '              </tr>';
    }
//  }
?>
...
// start change for pec and zarinpal payment modules
  if (($form_action_url == 'https://www.pec24.com/pecpaymentgateway/default.aspx') OR ($form_action_url == 'https://www.zarinpal.com/users/pay_invoice/')) {
  echo '</form>';
  } else {
...
// end change for pec and zarinpal payment modules
?>
